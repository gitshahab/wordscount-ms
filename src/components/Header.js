  ...
  return (
    ...
    <nav className='border-b-2'>
        <div className='flex justify-center item-center mx-2 my-2 px-1 py-1 md:px-2 md:py-2'>
            ...
            <Link to='/' className=''>
                <img data-testid="headerImg" className='size-7 md:size-9 mt-1 dark:ring-2 ring-white' src={Logo} alt="logo" />
            </Link>
            </div>
            <Link to='/' data-testid="headerHomeText" onClick={() => setBold(!bold)} className={`${bold || isPath.pathname === "/" ? `bg-black text-white rounded-full dark:bg-white dark:text-black` : ""} px-2 py-2 text-gray-900 ${!bold && `dark:text-gray-100`} text-sm md:text-lg font-semibold`} >Home</Link>
